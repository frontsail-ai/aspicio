...
| E2E                | Playwright e2e suites: `apps/demo`, `apps/react-example`, `apps/elements-example`, `apps/vue-example` |
...
(`packages/elements`), with `packages/react` and `packages/vue` as thin
veneers over them; the API Worker and MCP server expose the same
pipeline to agents.
