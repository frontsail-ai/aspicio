...
| Bindings & apps | `packages/elements/`, `packages/react/`, `apps/demo/`         | UI opinion lives here, never in core (INV-1). The Lit web components are the one implementation of embed UI; `packages/react` is a thin veneer over them, and Vue/Svelte consume them directly |
