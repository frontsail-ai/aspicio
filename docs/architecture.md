...
| Bindings & apps | `packages/elements/`, `packages/react/`, `packages/vue/`, `apps/demo/` | UI opinion lives here, never in core (INV-1). The Lit web components are the one implementation of embed UI; `packages/react` and `packages/vue` are thin veneers over them, and Svelte consumes them directly |
